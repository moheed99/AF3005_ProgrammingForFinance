...
# Page config
st.set_page_config(
    page_title="Harry Potter Financial Mystics - Multi Dashboard",
    page_icon="🧙‍♂️",
    layout="wide",
    initial_sidebar_state="expanded",
)

# Inject fonts & CSS for Harry potter vibe with animations
def remote_css(url):
...
@keyframes glow {
  ...
  50% {
    box-shadow: 0 0 25px 15px #fedd00;
  }
  100% {
    box-shadow: 0 0 10px #fedd00;
  }
...
.sidebar .sidebar-content {
  background-image: url('https://i.pinimg.com/originals/34/02/7b/34027b36a4d74bdfd650030a445a660e.jpg');
  background-size: cover;
...
# Magical wand unicode for flair
WAND = "🪄"

# Sidebar for page navigation and data input
...
# Placeholders for dataframe and ticker data storage in session_state
...
def welcome_page():
    ...
    st.markdown(
        ...
        <li>✨ Enjoy magical animations and a Harry Potter inspired theme</li>
        </ul>
        """
        , unsafe_allow_html=True)
    ...
    st.markdown("### Select Data Source in Sidebar")
    st.markdown("Go to **Data Exploration** page after loading your data.")
    ...
    if df is not None:
        ...
        if numeric_cols:
            ...
            for col in numeric_cols:
                fig, ax = plt.subplots()
                sns.histplot(df[col].dropna(), kde=True, color="#fedd00", ax=ax)
                ax.set_facecolor('#2c1a4f')
                ax.spines['bottom'].set_color('white')
                ax.spines['left'].set_color('white')
                ax.tick_params(axis='x', colors='white')
                ax.tick_params(axis='y', colors='white')
                ax.title.set_color('#fedd00')
                st.pyplot(fig)
        else:
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=ticker_data['Date'], y=ticker_data['Close'], mode='lines+markers', name='Close Price', line=dict(color='#fedd00')))
        fig.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font_color='#fedd00')
...
def ml_models():
    ...
    if dataset_choice == "Uploaded Dataset":
        data_df = df.copy()
    else:
        data_df = ticker_data.copy()
...
# Footer with animated magical button
st.markdown("---")
...
with col2:
    ...
    .magic-btn {
        ...
        border: none;
        cursor: pointer;
        box-shadow: 0 0 20px #fedd00;
        animation: pulseGlow 2.5s infinite alternate;
        transition: transform 0.3s ease;
    }
    .magic-btn:hover {
        box-shadow: 0 0 40px 10px #fedd00;
        transform: scale(1.2) rotate(-5deg);
    }
    @keyframes pulseGlow {
        0% {box-shadow: 0 0 20px #fedd00;}
        50% {box-shadow: 0 0 40px 20px #fedd00;}
        100% {box-shadow: 0 0 20px #fedd00;}
    }
    </style>
