...
import time
import random
# import requests # No longer needed as Unsplash is removed

# --- Constants for Publicly Available Assets (Examples) ---
# YOU SHOULD REPLACE THESE WITH YOUR PREFERRED, FREELY AVAILABLE ASSETS
# Verify licenses if using for more than personal projects.
...
# Static Background Images (Fallback if video fails or as primary if no video for a theme)
# YOU SHOULD PROVIDE THESE URLs for each house
DEFAULT_STATIC_BACKGROUND_URL = "https://i.imgur.com/6hZ0Q1M.jpg" # Hogwarts Castle
GRYFFINDOR_STATIC_BACKGROUND_URL = "https://i.imgur.com/9gD0x0g.jpg" # Gryffindor Common Room
SLYTHERIN_STATIC_BACKGROUND_URL = "https://i.imgur.com/pwxpNrN.jpg" # Slytherin Common Room
HUFFLEPUFF_STATIC_BACKGROUND_URL = "https://i.imgur.com/B9Y4S9A.jpg" # Hufflepuff Common Room
RAVENCLAW_STATIC_BACKGROUND_URL = "https://i.imgur.com/s6p0NKy.jpg" # Ravenclaw Common Room
...
house_themes = {
    "None": { 
        ...
        "background_image_url": DEFAULT_STATIC_BACKGROUND_URL # Added for static image
    },
    "Gryffindor": {
        ...
        "background_image_url": GRYFFINDOR_STATIC_BACKGROUND_URL
    },
    "Slytherin": {
        ...
        "background_image_url": SLYTHERIN_STATIC_BACKGROUND_URL
    },
    "Hufflepuff": {
        ...
        "background_image_url": HUFFLEPUFF_STATIC_BACKGROUND_URL
    },
    "Ravenclaw": {
        ...
        "background_image_url": RAVENCLAW_STATIC_BACKGROUND_URL
    }
}
...
# Use the static background image from the theme
...
.stApp::before {{
    ...
    width: 100vw;
    height: 100vh;
    background-image: url('https://i.imgur.com/WJIc0JL.gif'); /* Subtle sparkle overlay */
...
@keyframes neonBorderMove {{
  0% {{ border-image-source: linear-gradient(45deg, {theme_config["primary"]}, {theme_config["secondary"]}, {theme_config["primary"]}); }}
  50% {{ border-image-source: linear-gradient(45deg, {theme_config["secondary"]}, {theme_config["primary"]}, {theme_config["secondary"]}); }}
  100% {{ border-image-source: linear-gradient(45deg, {theme_config["primary"]}, {theme_config["secondary"]}, {theme_config["primary"]}); }}
}}
@keyframes pulseGlow {{
  0% {{ box-shadow: 0 0 8px rgba({primary_rgb_css},0.7), inset 0 0 8px rgba({primary_rgb_css},0.5), 0 0 15px {theme_config["primary"]}; }}
  50% {{ box-shadow: 0 0 12px rgba({primary_rgb_css},0.9), inset 0 0 12px rgba({primary_rgb_css},0.7), 0 0 25px {theme_config["primary"]}, 0 0 10px {theme_config["secondary"]}; }}
  100% {{ box-shadow: 0 0 8px rgba({primary_rgb_css},0.7), inset 0 0 8px rgba({primary_rgb_css},0.5), 0 0 15px {theme_config["primary"]}; }}
}}
...
#video-background {{
    ...
    object-fit: cover; z-index: -2; opacity: 0.5; /* Video opacity over static image */
}}
h1, h2 {{
    font-family: 'Orbitron', 'Cinzel', sans-serif; color: {theme_config["primary"]};
    text-shadow: 0 0 10px {theme_config["primary"]}, 0 0 20px {theme_config["primary"]}, 0 0 25px rgba(255,255,255,0.4);
...
h3 {{
    font-family: 'MedievalSharp', 'Orbitron', cursive; color: {theme_config["secondary"]};
    text-shadow: 0 0 8px {theme_config["secondary"]}, 0 0 12px rgba({secondary_rgb_css}, 0.8); font-size: 1.8rem;
}}
.stButton>button {{
  background: {theme_config["button_bg"]}; border: 2px solid transparent; border-radius: 15px; color: white;
  font-family: 'Orbitron','Cinzel',serif; font-weight: bold; padding: 15px 35px; font-size: 18px; cursor: pointer;
  transition: all 0.4s ease; position: relative; z-index: 1; text-shadow: 0 0 5px black; letter-spacing: 1px;
  animation: neonBorderMove 4s ease-in-out infinite alternate, pulseGlow 2.5s infinite ease-in-out;
  background-origin: border-box; background-clip: padding-box, border-box;
}}
.stButton>button:hover {{
  background: {theme_config["button_hover_bg"]}; transform: scale(1.12) rotate(-1.5deg); color: white; animation: none;
  box-shadow: 0 0 35px 10px {theme_config["primary"]}, 0 0 20px {theme_config["secondary"]};
}}
.stButton>button:hover:before {{
  content: "✦"; position: absolute; top: -20px; left: -20px; font-size: 22px; color: {theme_config["secondary"]};
  animation: floatingSymbol 2.5s infinite ease-in-out; text-shadow: 0 0 6px {theme_config["secondary"]};
}}
.stButton>button:hover:after {{
  content: "✧"; position: absolute; bottom: -20px; right: -20px; font-size: 22px; color: {theme_config["primary"]};
  animation: floatingSymbol 2.5s infinite ease-in-out reverse; text-shadow: 0 0 6px {theme_config["primary"]};
}}
@keyframes floatingSymbol {{
  0% {{ transform: translate(0,0) rotate(0deg); opacity:.7; }} 50% {{ transform:translate(5px,-5px) rotate(180deg); opacity:1;}} 100% {{ transform:translate(0,0) rotate(360deg); opacity:.7; }}
}}
.stTextInput>div>input, .stSelectbox>div>div, .stMultiSelect>div>div, .stDateInput>div>div>input {{
  background-color:rgba(10,5,25,.8); color:{theme_config["text"]}; border-radius:12px; border:2px solid {theme_config["primary"]};
  padding:12px; font-size:16px; font-family:'Orbitron','Cinzel',serif; transition:all .3s ease; box-shadow:0 0 7px rgba({primary_rgb_css},.6);
}}
...
.stTextInput>div>input:focus, .stSelectbox>div>div:focus-within, .stMultiSelect>div>div:focus-within, .stDateInput>div>div>input:focus {{
  border:2px solid {theme_config["secondary"]}; box-shadow:0 0 15px {theme_config["primary"]},0 0 10px {theme_config["secondary"]}; background-color:rgba(15,10,35,.9);
}}
.st-emotion-cache-10oheav {{
    background-color:rgba(5,2,15,.97) !important; border:1px solid {theme_config["primary"]} !important;
    color:{theme_config["text"]} !important; font-family:'Orbitron','Cinzel',serif !important;
}}
.st-emotion-cache-trf2nb:hover {{
    background-color:rgba({primary_rgb_css},.35) !important; color:{theme_config["primary"]} !important;
}}
.css-1d391kg .sidebar .sidebar-content .css-1aumxhk {{
  color:{theme_config["primary"]} !important; text-shadow:0 0 7px {theme_config["primary"]}; font-family:'Orbitron','Cinzel',sans-serif;
}}
.css-1d391kg .sidebar .sidebar-content .stRadio label span{{
    color:{theme_config["text"]} !important; font-size:1.1em; font-family:'Orbitron','Cinzel',sans-serif;
}}
...
::-webkit-scrollbar-track{{background:rgba(5,2,15,.8);border-radius:10px;box-shadow:inset 0 0 6px rgba(0,0,0,.4)}}
::-webkit-scrollbar-thumb{{background:linear-gradient(60deg,{theme_config["primary"]},{theme_config["secondary"]});border-radius:10px;border:2px solid rgba(5,2,15,.8)}}
::-webkit-scrollbar-thumb:hover{{background:linear-gradient(60deg,{theme_config["secondary"]},{theme_config["primary"]});box-shadow:0 0 12px {theme_config["primary"]}}}
...
.animated-text-delay-1{{animation-delay:.25s}}
.animated-text-delay-2{{animation-delay:.5s}}
.animated-text-delay-3{{animation-delay:.75s}}
...
/* Shorthand CSS classes for condensed HTML */
.tc {{ text-align: center; }}
.fs11 {{ font-size: 1.1em; }}
.fs12 {{ font-size: 1.2em; }}
.i {{ font-style: italic; }}
.ad {{ animation: fadeInUp .9s forwards ease-out; opacity: 0;}} /* combines animated-text */
...
page_session_key = "selected_page_fm_v4"
...
def load_data_from_upload():
    ...
    uploaded_file = st.file_uploader("Upload financial dataset (CSV)", type=["csv"], key="csv_up_expl_p2", help="...")
    if uploaded_file:
        try:
        ...
        except Exception as e: st.error(f"Decryption failed: {e}"); st.markdown(f"""<div class="spell-note ad" style="border-color:#ff4444;"><p class="futuristic-text">Error: Stream corrupted. Verify format.</p></div>""", unsafe_allow_html=True)

def load_data_from_stock():
    st.markdown(f"### {CRYSTAL_BALL} Summon Quantum Market Signatures (Stocks)")
    tc,sc,ec = st.columns(3)
    with tc: ticker = st.text_input("Quantum Signature (e.g., MSFT)", value="GOOG", key="tk_stk_p2", help="...")
    with sc: start_date = st.date_input("Initial Chrono-Marker", pd.to_datetime("2023-01-01"), key="sd_stk_p2", help="...")
    with ec: end_date = st.date_input("Final Chrono-Marker", pd.to_datetime("today"), key="ed_stk_p2", help="...")
    if st.button(f"{ATOM} Summon Signatures", key="sum_stk_p2"):
        if not ticker: st.warning("Enter Quantum Signature."); return
        try:
            magical_loading(f"Calibrating for {ticker}..."); data = yf.download(ticker,start,end_date)
            if data.empty: st.error(f"No sigs for '{ticker}'."); st.session_state.ticker_data=None; st.markdown(f"""<div class="spell-note ad snerr"><p class="futuristic-text">No signal. Try diff sig/params.</p></div>""", unsafe_allow_html=True)
                ...
                st.markdown(f"""<div class="holodeck-container ad mt20"><h3 class="futuristic-text tc mb15" style="color:{theme_config['secondary']};">{ticker} Projection Summary {OWL}</h3><div class="flex-around-wrap gap10"><div class="sum-item"><h4 class="futuristic-text sum-title">Initiation</h4><p class="sum-value">${op:.2f}</p></div><div class="sum-item"><h4 class="futuristic-text sum-title">Termination</h4><p class="sum-value">${cp:.2f}</p></div><div class="sum-item"><h4 class="futuristic-text sum-title">Zenith</h4><p class="sum-value">${hp:.2f}</p></div><div class="sum-item"><h4 class="futuristic-text sum-title">Nadir</h4><p class="sum-value">${lp:.2f}</p></div><div class="sum-item"><h4 class="futuristic-text sum-title">Delta</h4><p class="sum-value" style="color:{cc};font-weight:bold;">{pc:.2f}%</p></div></div></div>""", unsafe_allow_html=True)
...
def reset_data():
    if st.button(f"{WAND} Evanesco (Purge Data)", key="reset_dat_p2"):
...
def welcome_page(): # Condensed for brevity
    ...
    with cm:
        ...
        with cols_btn_center[1]:
            if st.button(f"{ROCKET} Dive into Financial Cosmos! {ATOM}", key="dive_welcome_p2", use_container_width=True):
        ...
        if not st.session_state.get("sorting_complete", False):
            ...
            un_input = st.text_input("Chronomancer designation:",key="wiz_name_welcome_p2",value=un_val,help="...")
            if un_input: st.session_state.user_name = un_input
            if st.session_state.user_name:
                with st.expander(f"🌌 Oracle Calibration for {st.session_state.user_name}...", expanded=True):
                    ...
                    fq_val = st.selectbox("Chrono-crystal protocol:",["Invest (G)","Multiply (S)","Sustain (H)","Research (R)"],index=None,key="sort_q_welcome_p2",placeholder="Select prime directive...",help="...")
                    if st.button("Activate Oracle!",key="act_oracle_welcome_p2"):
                        if fq_val:
    ...
    cfeat,cfort = st.columns(2) # Define cfeat and cfort
    with cfeat: # Use cfeat
        with st.expander(f"{ATOM} Advanced Holo-Features {ATOM}",expanded=False):st.markdown(f"""<div class="ad futuristic-text"><ul><li>{SPELL_BOOK} Data Scroll Decryption</li><li>{GEAR} Predictive AI</li><li>{CRYSTAL_BALL} Holo-Visualizations</li><li>{ROCKET} Personalized Quantum Field</li></ul></div>""",unsafe_allow_html=True)
        with st.expander(f"{OWL} Hogwarts Frequencies & Protocols {OWL}",expanded=False):st.markdown(f"""<div class="futuristic-text"><h4>Gryffindor🦁</h4><p>Bold investors.</p><h4>Slytherin🐍</h4><p>Ambitious strategists.</p><h4>Hufflepuff🦡</h4><p>Sustainable growth.</p><h4>Ravenclaw🦅</h4><p>Analytical.</p></div>""",unsafe_allow_html=True)
    with cfort: # Use cfort
        ...
        with st.form("fortune_form_welcome_p2"):fi_val=st.text_input("Binary query to Oracle:",key="fortune_q_welcome_p2",placeholder="Investments achieve singularity?",help="...");csb_val=st.form_submit_button(f"{ATOM} Query Oracle")
        if csb_val:
    ...
    cols_c=st.columns(4);cs_l=[{"t":"Data Influx","i":POTION,"d":"Interface CSVs/tickers."},{"t":"Holo-Charting","i":CRYSTAL_BALL,"d":"Weave visual algorithms."},{"t":"Predictive AI","i":GEAR,"d":"Deploy ML."},{"t":"Quantum Observatory","i":ATOM,"d":"Interpret real-time."}]
    for i,s_val in enumerate(cs_l):with cols_c[i]:st.markdown(f"""<div class="spell-note ad ad-delay-{i+1} futuristic-text curriculum-card"><div><h4 class="tc" style="color:{theme_config['primary']};"><span class="fs15">{s_val['i']}</span> {s_val['t']}</h4><p class="fs09">{s_val['d']}</p></div><p class="tc i fs08" style="color:{theme_config['secondary']};">Integrate protocol...</p></div>""",unsafe_allow_html=True)
    st.markdown("<hr class='ad'>",unsafe_allow_html=True)
    ...
    cols_g=st.columns(3);art_l=[{"n":"Temporal Ledger","img":ARTIFACT_GIF_1_URL,"d":"Reveals historical waves."},{"n":"Quantum Abacus","img":ARTIFACT_GIF_2_URL,"d":"Transmutes data streams."},{"n":"Oracle Holo-Projector","img":ARTIFACT_GIF_3_URL,"d":"Simulates market futures."}]
    for i,art_val in enumerate(art_l):with cols_g[i]:st.markdown(f"""<div class="holodeck-container ad ad-delay-{i+1} futuristic-text artifact-card tc"><div><img src="{art_val['img']}" width="90%" class="artifact-img" alt="{art_val['n']}"/><h4 style="color:{theme_config['primary']};">{art_val['n']}</h4><p class="fs09">{art_val['d']}</p></div></div>""",unsafe_allow_html=True)

def data_exploration(): # Further condensed
    ...
    t1,t2=st.tabs([f"{SPELL_BOOK} CSVs",f"{ATOM} Stocks"]);with t1:load_data_from_upload();with t2:load_data_from_stock()
    st.markdown("<hr class='ad'>",unsafe_allow_html=True);reset_data()
    df,td=st.session_state.get('df'),st.session_state.get('ticker_data')
    if df is not None:
        st.markdown(f"""<h2 class="futuristic-text tc ad">Decrypted Glyphs</h2>""",unsafe_allow_html=True);st.dataframe(df.head(5)) # Shortened head
        st.markdown(f"""<h3 class="futuristic-text tc ad">Quantum Insights</h3>""",unsafe_allow_html=True);cs,cst=st.columns(2)
        with cs:st.markdown(f"""<div class="spell-note ad futuristic-text"><h4>Stats Resonance</h4></div>""",unsafe_allow_html=True);st.dataframe(df.describe().T.head(3)) # Transposed and shortened
        with cst:st.markdown(f"""<div class="spell-note ad futuristic-text"><h4>Data Weave</h4></div>""",unsafe_allow_html=True);st.dataframe(pd.DataFrame({'F':df.columns,'T':df.dtypes.astype(str)}).head(3)) # Shortened
        st.markdown(f"""<h3 class="futuristic-text tc ad">Holo-Viz</h3>""",unsafe_allow_html=True);num_c,cat_c=df.select_dtypes(np.number).columns.tolist(),df.select_dtypes('object').columns.tolist()
        if num_c:viz_s=st.selectbox("Viz Algo:",["Density","Correlation","Harmonics"],index=None,key="viz_df_p3",placeholder="Select...");st.info(f"{viz_s} plotting logic here...") # Extremely condensed
        else:st.info("No numeric data.")
    elif td is not None:
        st.markdown(f"""<h2 class="futuristic-text tc ad">Market Readings</h2>""",unsafe_allow_html=True);st.dataframe(td.head(5)) # Shortened
        ...
        with t1:st.info("Price chart logic...") # Extremely condensed
        with t2:st.info("Volume chart logic...")
        with t3:st.info("Candlestick chart logic...")
...
# Add some missing CSS for condensed HTML elements if you used them:
condensed_css = """
<style>
...
.gap10 { gap: 10px; }
.flex-around-wrap { display: flex; justify-content: space-around; flex-wrap: wrap; }
.sum-item { text-align: center; padding: 10px; background: rgba(var(--primary-rgb-css-val, 254,221,0), 0.1); border-radius: 8px; } /* Fallback color for primary_rgb_css */
.sum-title { color: var(--text-color-val, #eee7db); font-size:0.9em; margin-bottom:5px; } /* Fallback for text */
.sum-value { font-size: 1.4rem; color: var(--primary-color-val, #fedd00); } /* Fallback for primary */
.snerr { border-color: #ff4444 !important; } /* Ensure error spell note border is red */
...
.artifact-img { border-radius: 10px; margin-bottom: 10px; border: 1px solid var(--secondary-color-val, #662d91); box-shadow: 0 0 8px var(--secondary-color-val, #662d91); } /* Fallback color for secondary */
.no-data-img { width:150px; margin-top:15px; border-radius:10px; opacity:0.7; }
.placeholder-gif { width:200px; margin-top:20px; border-radius:10px; opacity:0.8; }

/* Update CSS variables based on theme */
:root {{
    --primary-rgb-css-val: {primary_rgb_css};
    --text-color-val: {theme_config["text"]};
    --primary-color-val: {theme_config["primary"]};
    --secondary-color-val: {theme_config["secondary"]};
}}
</style>
"""
st.markdown(condensed_css, unsafe_allow_html=True)
