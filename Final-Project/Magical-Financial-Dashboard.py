...
.holodeck-container > * {{position:relative;z-index:2;}} /* Corrected: ensured semicolon */
...
.tc{{text-align:center}} .fs11{{font-size:1.1em}} .fs12{{font-size:1.2em}} .i{{font-style:italic}}
...
# Check if it's a placeholder by seeing if "YOUR_" is in the string, or if it's literally the default placeholder constant
...
page_session_key = "selected_page_fm_v5" # New unique key
...
current_page_selection = st.sidebar.radio(
    ...
    key="sidebar_page_selector_v5", # New unique key
...
def load_data_from_upload():
    ...
    uploaded_file = st.file_uploader("Upload financial dataset (CSV)", type=["csv"], key="csv_up_expl_p3", help="...")
    if uploaded_file:
        try:
...
def load_data_from_stock():
    ...
    with tc: ticker = st.text_input("Quantum Signature (e.g., MSFT)", value="GOOG", key="tk_stk_p3", help="...")
    with sc: start_date = st.date_input("Initial Chrono-Marker", pd.to_datetime("2023-01-01"), key="sd_stk_p3", help="...")
    with ec: end_date = st.date_input("Final Chrono-Marker", pd.to_datetime("today"), key="ed_stk_p3", help="...")
    if st.button(f"{ATOM} Summon Signatures", key="sum_stk_p3"):
        if not ticker: st.warning("Enter Quantum Signature."); return
        try:
            magical_loading(f"Calibrating for {ticker}..."); data = yf.download(ticker,start_date,end_date) # Corrected start variable
            if data.empty: st.error(f"No sigs for '{ticker}'."); st.session_state.ticker_data=None; st.markdown(f"""<div class="spell-note ad snerr"><p class="futuristic-text">No signal. Try diff sig/params.</p></div>""", unsafe_allow_html=True)
                ...
                summary_html = f"""
                <div class="holodeck-container ad mt20">
                    <h3 class="futuristic-text tc mb15" style="color:{theme_config['secondary']};">{ticker} Projection Summary {OWL}</h3>
                    <div class="flex-around-wrap gap10">
                        <div class="sum-item"><h4 class="futuristic-text sum-title">Initiation</h4><p class="sum-value">${op:.2f}</p></div>
                        <div class="sum-item"><h4 class="futuristic-text sum-title">Termination</h4><p class="sum-value">${cp:.2f}</p></div>
                        <div class="sum-item"><h4 class="futuristic-text sum-title">Zenith</h4><p class="sum-value">${hp:.2f}</p></div>
                        <div class="sum-item"><h4 class="futuristic-text sum-title">Nadir</h4><p class="sum-value">${lp:.2f}</p></div>
                        <div class="sum-item"><h4 class="futuristic-text sum-title">Delta</h4><p class="sum-value" style="color:{cc};font-weight:bold;">{pc:.2f}%</p></div>
                    </div>
                </div>"""
...
def reset_data():
    if st.button(f"{WAND} Evanesco (Purge Data)", key="reset_dat_p3"): # Unique key
...
def welcome_page(): # Further condensed for brevity, ensure all unique keys
    ...
    with cm:
        ...
        with cols_btn_center[1]:
            if st.button(f"{ROCKET} Dive into Financial Cosmos! {ATOM}", key="dive_welcome_p3", use_container_width=True): # Unique key
        ...
        if not st.session_state.get("sorting_complete", False):
            ...
            un_input_val = st.text_input("Chronomancer designation:",key="wiz_name_welcome_p3",value=un_val,help="...") # Unique key
            ...
            if st.session_state.user_name:
                with st.expander(f"🌌 Oracle Calibration for {st.session_state.user_name}...", expanded=True):
                    ...
                    fq_val = st.selectbox("Chrono-crystal protocol:",["Boldly invest (Gryffindor)","Strategically multiply (Slytherin)","Sustainably grow (Hufflepuff)","Research application (Ravenclaw)"],index=None,key="sort_q_welcome_p3",placeholder="Select prime directive...",help="...") # Unique key
                    if st.button("Activate Oracle!",key="act_oracle_welcome_p3"): # Unique key
                        if fq_val:
    ...
    with cfeat:
        with st.expander(f"{ATOM} Advanced Holo-Features {ATOM}",expanded=False):st.markdown(f"""<div class="ad futuristic-text"><ul style="list-style-type:none;padding-left:0;"><li><span class="fs13">{SPELL_BOOK}</span> Data Scroll Decryption</li><li><span class="fs13">{GEAR}</span> Predictive AI</li><li><span class="fs13">{CRYSTAL_BALL}</span> Holo-Visualizations</li><li><span class="fs13">{ROCKET}</span> Personalized Field</li></ul></div>""",unsafe_allow_html=True) # Simplified list items
        with st.expander(f"{OWL} Hogwarts Frequencies & Protocols {OWL}",expanded=False):st.markdown(f"""<div class="futuristic-text"><h4>Gryffindor🦁</h4><p>Bold investors.</p><h4>Slytherin🐍</h4><p>Ambitious.</p><h4>Hufflepuff🦡</h4><p>Sustainable.</p><h4>Ravenclaw🦅</h4><p>Analytical.</p></div>""",unsafe_allow_html=True) # Further simplified
    with cfort:
...
def data_exploration():
    ...
    df,td=st.session_state.get('df'),st.session_state.get('ticker_data') # Renamed ticker_data to td for brevity
    if df is not None:
        ...
        if num_c:
            viz_s=st.selectbox("Viz Algo:",["Density","Correlation","Harmonics"],index=None,key="viz_df_p4",placeholder="Select..."); # Unique key
            if viz_s == "Density" and num_c: 
                sel_cols = st.multiselect("Select streams for Density Map:", num_c, default=num_c[:min(1,len(num_c))], key="hist_ms_p4") # Unique key
                for col in sel_cols: st.write(f"Histogram for {col}...") # Placeholder
            elif viz_s == "Correlation" and len(num_c)>1: st.write("Heatmap logic...") # Placeholder
            elif viz_s == "Harmonics" and cat_c and num_c: 
                sel_cat = st.selectbox("Select Category Stream:", cat_c, key="bar_cat_p4") # Unique key
                sel_num = st.selectbox("Select Value Stream:", num_c, key="bar_num_p4") # Unique key
                st.write(f"Bar chart for {sel_num} by {sel_cat}...") # Placeholder
        ...
        with t1: st.write("Price chart for ticker...") # Placeholder for Plotly
        with t2: st.write("Volume chart for ticker...") # Placeholder for Plotly
        with t3: st.write("Candlestick chart for ticker...") # Placeholder for Plotly
...
# (This was already provided in the previous response and assumed to be part of base_css_rules now)
